    ...
    if (sim.info.membranes[m] == DomainType.ellipsoid &&
      ...
      for (var i = 0; i < sim.buffer.nParticles; i++) {
        ...
        if (sim.buffer.isInMembrane(i, m)) {
          // Filter inner particles that are now illegally outside.
          if (!op && !oldDomain.containsVec(particle)) {
            // Apply projection.
            var projPos = _innerProjection(particle, newDomain, poff);
            sim.buffer.setParticleCoords(i, projPos);
          }
        } else if (!ip && newDomain.containsVec(particle)) {
          // Apply projection.
          var projPos = _outerProjection(particle, oldDomain, newDomain, poff);
          sim.buffer.setParticleCoords(i, projPos);
        }
      }

      // Project outer particles that are inside the movement bounds and that
      // are now illegally inside on the outer surface.
...
Vector3 _innerProjection(
  ...
  // Note that we use a unit direction vector so we can apply offset later.
  var ray =
      new Ray.originDirection(particle, particle / particle.length * -1.0);
...
Vector3 _outerProjection(Vector3 particle, EllipsoidDomain oldEllipsoid,
  ...
  var direction = particle - oldEllipsoid.center;
  var ray = new Ray.originDirection(oldEllipsoid.center, direction);
  ...
  // Return projected point with extra offset towards the new ellipsoid center.
  var point = ray.at(t);
...
Tuple2<double, double> _ellipsoidProjection(Ray ray, EllipsoidDomain e) {
    ...
    var B = bc * xO * xD + ac * yO * yD + ab * zO * zD;
